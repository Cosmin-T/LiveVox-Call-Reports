...
def reporting_air():
    ...
    df.to_excel(FILE_OUTPUT_AIR, index=False, header=None)
    logging.info('DataFrame Exported to Excel')


def apply_filters_to_excel_air():
    try:
        workbook = load_workbook(FILE_OUTPUT_AIR)
        ...
        for row in sheet.iter_rows(min_row=2):  # Assuming row 1 is the header
            lv_result_cell = row[lv_result_col_index - 1]  # -1 because iter_rows is 0-indexed
